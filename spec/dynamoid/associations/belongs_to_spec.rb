...
describe Dynamoid::Associations::BelongsTo do
  context "has many" do
    ...
    it "behaves like the object it is trying to be" do
      ...
      expect(
        subscription.magazine.publish(advertisements: 10, free_issue: true)
      ).to eq 20
  ...
  describe "assigning" do
    context "has many" do
      ...
      it "re-associates model on this side" do
        ...
        expect do subscription.magazine = magazine_new end.to change {
          subscription.magazine.target
        }.from(magazine_old).to(magazine_new)
      end
      ...
      it "re-associates model on that side" do
        ...
        expect do subscription.magazine = magazine_new end.to change {
          magazine_new.subscriptions.target
      ...
      it "deletes previous model from association" do
        ...
        expect do subscription.magazine = magazine_new end.to change {
          magazine_old.subscriptions.to_a
      ...
      it "does not store the same object on that side" do
        ...
        expect(magazine.subscriptions.target[0].object_id).to_not eq(
    ...
    context "has one" do
      ...
      it "re-associates model on this side" do
        ...
        expect do sponsor.magazine = magazine_new end.to change {
          sponsor.magazine.target
        }.from(magazine_old).to(magazine_new)
      end
  ...
  context "set to nil" do
    ...
    it "overrides previous saved value" do
      ...
      expect do
        ...
        subscription.save!
      end.to change { Subscription.find(subscription.id).magazine.target }.from(
        magazine
      ).to(nil)
    end
    ...
    it "updates association on the other side" do
      ...
      expect do
        subscription.magazine = nil
        subscription.save!
      end.to change { Magazine.find(magazine.title).subscriptions.to_a }.from(
        [subscription]
      ).to([])
    end
  end
  ...
  describe "#delete" do
    it "clears association on this side" do
      ...
      expect do subscription.magazine.delete end.to change {
        subscription.magazine.target
    ...
    context "has many" do
      ...
      it "clears association on that side" do
        expect do subscription.magazine.delete end.to change {
          magazine.subscriptions.target
        }.from([subscription]).to([])
      end

      it "persists changes on that side" do
        expect do subscription.magazine.delete end.to change {
          Magazine.find(magazine.title).subscriptions.target
        }.from([subscription]).to([])
      end
    end
    ...
    context "has one" do
      ...
      it "clears association on that side" do
        expect do sponsor.magazine.delete end.to change {
          magazine.sponsor.target
        }.from(sponsor).to(nil)
      end
