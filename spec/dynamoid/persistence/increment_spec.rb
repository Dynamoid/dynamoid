...
RSpec.describe Dynamoid::Persistence do
  ...
  describe '#increment!' do
    ...
    it "raises UnknownAttribute when an attribute name isn't declared as a field" do
      obj = klass.create!
      expect { obj.increment!(:unknown) }.to raise_error(Dynamoid::Errors::UnknownAttribute)
    end
    ...
    context 'when :touch option passed' do
      it 'updates updated_at when touch: true' do
        ...
        expect { obj.increment!(:age) }.not_to change { obj.reload.updated_at }
        expect { obj.increment!(:age, touch: true) }.to change { obj.reload.updated_at }
      end
    ...
    describe 'primary key validation' do
      ...
      context 'composite key' do
        ...
        it 'requires sort key to be specified' do
          ...
          expect { obj.increment!(:age) }.to raise_error(Dynamoid::Errors::MissingRangeKey)
        end
      end
    end
