...
describe Dynamoid::Tasks::Database do
  ...
  describe '#create_tables' do
    before(:each) do
      ...
      @models = Dynamoid.included_models.select{ |m| not m.base_class.try(:name).blank? }.uniq(&:table_name)
      ...
      # We need this gone.
      existing_tables = @models.map{ |m| m.table_name } & Dynamoid.adapter.list_tables
    ...
    context "when the tables don't already exist" do
      it 'should create tables' do
        expect(Dynamoid.adapter.list_tables).not_to include( *@models.map{ |m| m.table_name } )
        results = Dynamoid::Tasks::Database.create_tables
        expect(Dynamoid.adapter.list_tables).to include( *@models.map{ |m| m.table_name } )
        expect(results[:created]).to include( *@models.map{ |m| m.table_name } )
      end
    end
